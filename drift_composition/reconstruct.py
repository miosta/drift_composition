...
class BandedMatrix:
    ...
    def dot(self, x):
        ...
        """
        m_shape = self._mat.shape + (1,) * (len(x.shape) - 1)
        m = self._mat.reshape(m_shape) * x.reshape(-1, *x.shape)
...
def _construct_volume_factors(xi, m, order, dtype):
    ...
    for n in range(order):
        beta[:, n] = np.diff(xi ** (m + n + 1)) / (m + n + 1.0)
...
def compute_FV_weights(xe, xj, m, stencil, max_deriv=None, dtype="f8"):
    ...
    if max_deriv is None:
        ...
        raise ValueError(
            "Maximum derivative must be less than the order of the polynomial fitted"
        )
